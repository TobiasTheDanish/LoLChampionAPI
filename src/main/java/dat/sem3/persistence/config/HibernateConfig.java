package dat.sem3.persistence.config;
...
public class HibernateConfig {
    ...
    private static EntityManagerFactory buildEntityFactoryConfig(String dbName) {
        try {
            Configuration configuration = new Configuration();

            Properties props = new Properties();

            props.put("hibernate.connection.url", String.format("jdbc:postgresql://localhost:5432/%s?currentSchema=public", dbName));
    ...
    private static void getAnnotationConfiguration(Configuration configuration) {
        ...
        configuration.addAnnotatedClass(ChampionDetails.class);
    }

    public static EntityManagerFactory getEntityManagerFactoryConfig(String dbName) {
        if (entityManagerFactory == null) entityManagerFactory = buildEntityFactoryConfig(dbName);
